...
define([
    ...
    var app = {
        ...
        // Application base URL to use in API docs
        baseUrl: 'http://localhost:8080',
        protocolRelativebaseUrl: '//localhost:8080',
        // Max input text size.
        maxinput: 50000,
    ...
    return _.extend(app, {
        ...
        setTitle: function (title) {
            title = (title === undefined || title === '') ? 'neji server' : title + ' - neji';
            $(document).attr('title', title);
        },

        // Alert messages helper
        alert: function (options) {
            options = _.isString(options) ? {msg: options} : options;
            this.trigger('alert:show', options);
        },
