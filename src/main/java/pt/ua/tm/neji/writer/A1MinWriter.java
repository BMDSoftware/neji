...
public class A1MinWriter extends BaseWriter {

    /**
     * {@link Logger} to be used in the class.
     */
    private static Logger logger = LoggerFactory.getLogger(A1MinWriter.class);
    private int offset;
    private StringBuilder content;
    private int processedAnnotations;
    private int normalizationCounter;

    public A1MinWriter() throws NejiException {
        ...
        for (TreeNode<Annotation> node : nodes) {
            ...
            for (Identifier id : data.getIDs()) {                
                ...
                if (!groups.containsKey(group)) {
                    ...
                    ids.add(id.toStringMinimized());
                    groups.put(group, ids);
                } else {
                    groups.get(group).add(id.toStringMinimized());
                }
            }
